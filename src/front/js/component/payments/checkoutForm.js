  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { error, paymentIntent } = await stripe.confirmPayment({
        ...
        redirect: "if_required", // 🔥 se queda en la misma página
      });
      ...
      if (error) {
        ...
        // 💳 Éxito total del pago
        setMessage("Payment successful!");
        setShowSuccessModal(true); // 👉 abre tu modal moderno
        setPaymentMethod("stripe");
        setPaymentMessage("Your payment was processed successfully!");
      }
    } catch (err) {
